...
- You're using a Yellow iPhone 14 or Yellow iPhone 14 Plus on iOS 16.3 (20D50), in which case you should install `LuiseInstallerX-20D50.ipa` from [here](https://github.com/xztimee/LuiseInstallerX/releases/latest).

Additionally, if a file exists at `/LuiseInstallerX.app/kernelcache`, LuiseInstallerX will use that file instead of downloading the kernelcache. This can be useful if you have a slow or unreliable internet connection, or happen to have a device and version combination that has no public kernelcache available.

> Why does LuiseInstallerX say "failed to install persistence helper" using the indirect method?
...
## Building
LuiseInstallerX is a regular Xcode project, but the project also contains a build script. To build it and produce an IPA, simply run the `build.sh` script in the root of the project. This will build the project and produce an IPA in the root of the project.

## Credits
LuiseInstallerX wouldn't have been possible without the work of the following people:
...
* [aaronp613](https://x.com/aaronp613) for the LuiseInstallerX icon
